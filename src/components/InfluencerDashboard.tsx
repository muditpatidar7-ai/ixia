...
export function InfluencerDashboard() {
  ...
  return <main className="min-h-screen bg-cloud-gray px-5 py-10 sm:px-8"><div className="mx-auto max-w-6xl space-y-8">
    <header className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-steel-blue">Influencer dashboard</p><h1 className="mt-2 text-4xl font-semibold text-forest-green">Welcome, {profile.full_name}</h1></div><div className="flex items-center gap-3"><span className={`rounded-full px-4 py-2 text-sm font-semibold ${profile.is_admin_verified ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-800"}`}>{profile.is_admin_verified ? "Verified" : "Pending Verification"}</span><button type="button" onClick={() => void logout()} className="rounded-xl bg-forest-green px-4 py-2 font-semibold text-white transition hover:bg-forest-green/90 active:bg-terracotta">Log out</button></div></header>
